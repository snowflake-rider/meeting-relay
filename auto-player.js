...
const send=payload=>fetch('/send',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({to:'sender',session,payload,channel}),keepalive:true});
...
(async()=>{while(true){try{if(running){const r=await fetch('/poll?role=receiver&session='+session+'&channel='+channel);if(!r.ok)throw Error('서버 연결 확인 필요');for(const m of await r.json())if(m.payload.offer)await accept(m.payload);}}catch(e){statusEl.textContent=e.message;clear();}await new Promise(r=>setTimeout(r,500));}})();
